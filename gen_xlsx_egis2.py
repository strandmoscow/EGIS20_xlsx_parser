...
def export_subsystem_to_excel_egis2(sys_code, output_dir):
    # Подключение к БД
    engine = init_conn()

    # Получаем информацию о подсистеме через функцию
    with engine.connect() as conn:
        query = text('SELECT system_name FROM "egis2.0_test".egis_2_systems WHERE system_code = :sys_code')
    ...
    query = text('SELECT * FROM "egis2.0_test".get_egis2_subsystem_report(:sys_name)')
    ...
    create_xlsx(output_file, sys_code, sys_desc, df, 2)
